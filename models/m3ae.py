    ...
    def forward(self, inputs, deterministic=False, padding_mask=None):
        ...
        if padding_mask is not None:
            ...
            if attention.is_cuda:
                attention = torch.where(padding_mask > 0, torch.tensor(-1e7).to(torch.device('cuda:0')), attention)
            else:
...
class MaskedMultimodalAutoencoder(nn.Module):
    ...
    def __init__(self, text_vocab_size, config_updates=None):
        ...
        self.text_embedding = nn.Embedding(self.text_vocab_size, self.config.emb_dim)
    ...
    def forward_representation(self, image, text, text_padding_mask, deterministic=False):
        ...
        batch_size = image.shape[0] if image is not None else text.shape[0]
        cls_token = self.cls_token.expand(batch_size, 1, self.config.emb_dim)
        input_tensors = [cls_token]
        padding_masks = [torch.zeros((batch_size, 1), dtype=torch.float32).to(device)]
        
        if image is not None:
            image_x = (
                self.image_embedding(image)
                + torch.tensor(get_2d_sincos_pos_embed(self.config.emb_dim, image.shape[1])).to(device)
                + self.get_type_embedding('encoder_image_type_embedding')
            )
            input_tensors.append(image_x)
            padding_masks.append(torch.zeros((batch_size, image.shape[1]), dtype=torch.float32).to(device))
        if text is not None:
            text_x = (
                    self.text_embedding(text)
                    + torch.tensor(get_1d_sincos_pos_embed(self.config.emb_dim, text.shape[1])).to(device)
...
def get_image(filename, filename2=None, mix_lambda=1):
    ...
    if filename2 == None:
        ...
        image_tensor1 = preprocess(img1)

        img2 = Image.open(filename2)
        image_tensor2 = preprocess(img2)
...
if __name__ == "__main__":
    ...
    ckpt_path = "/data1/zhangxiaohui/OGM-GE/m3ae_large.pth"
    ...
    missing, unexcepted = model.load_state_dict(loaded_dict)
    ...
    token_path = "/data1/zhangxiaohui/MVSA_Single/text_token/train_token/5_token.npy"
    pm_path = "/data1/zhangxiaohui/MVSA_Single/text_token/train_token/5_pm.npy"
    image_path = "/data1/zhangxiaohui/MVSA_Single/visual/train_imgs/5.jpg"
    tokenized_caption = np.load(token_path)
    ...
    with torch.no_grad():
        representation = model.forward_representation(None, tokenized_caption, padding_mask)
